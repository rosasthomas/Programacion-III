<?php
    ...
    class Media{
        ...
        public function BorrarUno($request, $response, $args) {
            ...
            $usu = json_decode($datos['usuario']);
            if($usu->perfil == 'propietario'){
                $media= new Media();
                $media->id=$id_media;
        ...
        public function ModificarUno($request, $response, $args) {
            $datos = $request->getParsedBody();
            $info = json_decode($datos['datos']);
            
            if($info->perfil == 'propietario' || $info->perfil == 'encargado'){
                $media= new Media();
                $media->id=$info->id;
                $media->color=$info->color;
                $media->marca=$info->marca;
                $media->precio=$info->precio;
                $media->talle=$info->talle;
